from openai_res import get_completion
...
2. Extract the following information:
   ...
   - Script Follow Score
   - Sales Pitch Score
   - Accomodation booking status

### Script Follow Segments
...
Follow this result format for response- \n
...
Info Extraction:
   ...
   Agent Name: %'Example - Rohan’%
   Accommodation Preference: %'Example - Single room with private bathroom’%
   Country: Example - %'USA’%
   University: Example - %'Harvard University’%
   Intake: Example - %'September 2024’%
   Budget: Example - %'$20,000 per year’%
   Self-funding or Loan: %'Example - Partial self-funding and partial education loan discussed’%
   Move-in Date: Example - %'August 15, 2024’%
   Script Follow Score: %'Example - 10/10’%
   Sales Pitch Score: %'Example - 8.5/10’%
   Accommodation booking status: %'Example - Not booked yet’%
...
Suggestions: Based on the transcript, suggested actions for the agent to improve the call.
     ...
     $Result Format-Ends Here$(don't include this heading)
...
prompt = f"""
 ...
 Analyze the agent's performance based on the provided script, using the given result format.
    ...
    ***here are the tasks -   {ai_prompt_uniacco}
